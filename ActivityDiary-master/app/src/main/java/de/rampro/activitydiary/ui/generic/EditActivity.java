...
public class EditActivity extends BaseActivity implements ActivityHelper.DataChangedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView mTextField = findViewById(R.id.myTextView);
        ...
        selectTimeButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showTimePickerDialog(mTextField);
            }
        });
...
//+++++++++++++
    private void showTimePickerDialog(TextView mTextField) {
        TimePickerDialog timePickerDialog = new TimePickerDialog(this,
                (view, hourOfDay, minute) -> {
                    // 在时间选择后的回调方法中进行处理
                    String selectedTime = String.format(Locale.getDefault(), "%02d:%02d", hourOfDay, minute);
                    // 在这里更新界面或执行其他操作
                    mTextField.setText("任务将于" + hourOfDay+"小时"+minute+"分钟后完成");
                }, 0, 0, true);
